...
const filteredOptions = computed(() => {
  ...
  return flattenedOptions.value.filter(option => 
    option.toLowerCase().includes(v.value.toLowerCase())
  )
})
...
<template>
  <ComboboxRoot v-model="v" class="relative">
    <ComboboxAnchor
      ...
      <img v-if="v" src="@/assets/icons/close.svg" alt="search" class="w-4 h-4 text-gray-400 cursor-pointer" @click="handleClear" />
    </ComboboxAnchor>
    ...
    <ComboboxContent
      ...
      <ComboboxViewport class="p-1">
        ...
        <ComboboxGroup>
          <!-- 虚拟滚动容器 -->
          <div v-bind="containerProps" class="max-h-[200px] overflow-auto">
